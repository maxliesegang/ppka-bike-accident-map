...
const getFillColor = (properties: AccidentProperties): string => {
  ...
  if (isSingleBikeAccident(properties)) return getColorForType('SINGLE_BIKE');
  if (isBikeOnlyAccident(properties)) return getColorForType('BIKE_ONLY');
  return getColorForType('DEFAULT_FILL');
};

const getRadius = (properties: AccidentProperties): number => {
  if (hasSevereInjuries(properties)) return getRadiusForType('SEVERE_INJURY');
  if (hasInjuries(properties)) return getRadiusForType('INJURY');
